...
public class ProxyServlet extends HttpServlet {
  ...
  protected static final String P_TARGET_URI_STATIC = "targetUriStatic";
  ...
  /** From the configured parameter "targetUri". */
  protected String targetUriStatic;
  protected URI targetUriObjStatic;//new URI(targetUri)
  protected HttpHost targetHostStatic;//URIUtils.extractHost(targetUriObj);
  ...
  protected void initTarget() throws ServletException {
    ...
    targetUriStatic = getConfigParam(P_TARGET_URI_STATIC);
    if (targetUriStatic == null)
      throw new ServletException(P_TARGET_URI_STATIC+" is required.");
    //test it's valid
    try {
      targetUriObjStatic = new URI(targetUriStatic);
    } catch (Exception e) {
      throw new ServletException("Trying to process targetUri init parameter: "+e,e);
    }
    targetHostStatic = URIUtils.extractHost(targetUriObjStatic);
  }
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
    ...
    if (servletRequest.getAttribute(ATTR_TARGET_URI) == null) {
      if(StringUtils.endsWithAny(servletRequest.getRequestURI(),"css","js","jpg","png","gif","woff","ttf")){
        servletRequest.setAttribute(ATTR_TARGET_URI, targetUriStatic);
      }else{
    ...
    if (servletRequest.getAttribute(ATTR_TARGET_HOST) == null) {
      if(StringUtils.endsWithAny(servletRequest.getRequestURI(),"css","js","jpg","png","gif","woff","ttf")) {
        servletRequest.setAttribute(ATTR_TARGET_HOST, targetHostStatic);
      }else{
